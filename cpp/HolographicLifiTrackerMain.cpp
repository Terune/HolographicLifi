...
HolographicLifiTrackerMain::HolographicLifiTrackerMain(const std::shared_ptr<DX::DeviceResources>& deviceResources) :
...
void HolographicLifiTrackerMain::SetHolographicSpace(HolographicSpace^ holographicSpace)
{
    ...
    m_locatabilityChangedToken =
        m_locator->LocatabilityChanged +=
            ref new Windows::Foundation::TypedEventHandler<SpatialLocator^, Object^>(
                std::bind(&HolographicLifiTrackerMain::OnLocatabilityChanged, this, _1, _2)
                );
    ...
    m_cameraAddedToken =
        m_holographicSpace->CameraAdded +=
            ref new Windows::Foundation::TypedEventHandler<HolographicSpace^, HolographicSpaceCameraAddedEventArgs^>(
                std::bind(&HolographicLifiTrackerMain::OnCameraAdded, this, _1, _2)
                );
    ...
    m_cameraRemovedToken =
        m_holographicSpace->CameraRemoved +=
            ref new Windows::Foundation::TypedEventHandler<HolographicSpace^, HolographicSpaceCameraRemovedEventArgs^>(
                std::bind(&HolographicLifiTrackerMain::OnCameraRemoved, this, _1, _2)
                );
        ...
        .then([this](std::shared_ptr<VideoFrameProcessor> videoProcessor)
    {
        ...
        return LifiTrackerProcessor::CreateAsync(m_videoFrameProcessor)
            .then([this](std::shared_ptr<LifiTrackerProcessor> faceProcessor)
        {
            m_LifiTrackerProcessor = std::move(faceProcessor);
        });
    });
...
void HolographicLifiTrackerMain::UnregisterHolographicEventHandlers()
{
...
HolographicLifiTrackerMain::~HolographicLifiTrackerMain()
{
...
void HolographicLifiTrackerMain::ProcessFaces(std::vector<BitmapBounds> const& faces, Windows::Media::Capture::Frames::MediaFrameReference^ frame, SpatialCoordinateSystem^ worldCoordSystem)
{
...
HolographicFrame^ HolographicLifiTrackerMain::Update()
{
    ...
    if (m_videoFrameProcessor && m_LifiTrackerProcessor)
    {
        m_trackingFaces = m_LifiTrackerProcessor->IsTrackingFaces();

        if (m_trackingFaces)
        {
            if (MediaFrameReference^ frame = m_videoFrameProcessor->GetLatestFrame())
            {
                ProcessFaces(m_LifiTrackerProcessor->GetLatestFaces(), frame, currentCoordinateSystem);
...
bool HolographicLifiTrackerMain::Render(Windows::Graphics::Holographic::HolographicFrame^ holographicFrame)
{
    if (!m_isReadyToRender)
    {
        return false;
...
void HolographicLifiTrackerMain::SaveAppState()
{
    // This sample does not save any state on suspend.
}

void HolographicLifiTrackerMain::LoadAppState()
{
...
void HolographicLifiTrackerMain::OnDeviceLost()
{
...
void HolographicLifiTrackerMain::OnDeviceRestored()
{
    auto initTasks = {
...
void HolographicLifiTrackerMain::OnLocatabilityChanged(SpatialLocator^ sender, Object^ args)
{
...
void HolographicLifiTrackerMain::OnCameraAdded(
...
void HolographicLifiTrackerMain::OnCameraRemoved(
    HolographicSpace^ sender,
